...
//scrap
const { hentai, quotesAnime } = require('../noapikey/scraper')
...
module.exports = async(chika, msg, m, ind, setting) => {
	...
	const command = chats.toLowerCase().split(' ')[0] || ''
        ...
        let dia = msg.quoted ? msg.quoted.sender : msg.mentionedJid && msg.mentionedJid[0] ? msg.mentionedJid[0] : false;
	...
	switch (command) {
		...
		break
            ...
            case prefix+'hentai': {
                ...
                chika.sendMessage(from, { video: { url: result.video_1 }, caption: `⭔ Title : ${result.title}\n⭔ Category : ${result.category}\n⭔ Mimetype : ${result.type}\n⭔ Views : ${result.views_count}\n⭔ Shares : ${result.share_count}\n⭔ Source : ${result.link}\n⭔ Media Url : ${result.video_1}` }, { quoted: msg })
            }
            break
            case prefix+'quotesanime': case prefix+'quoteanime': {
                ...
                let result = anu[Math.floor(Math.random(), anu.length)]
               chika.sendMessage(from, { text: `*QUOTES ANIME*\n\n_${result.quotes}_\n\nBy '${result.karakter}', ${result.anime}\n\n- ${result.up_at}`}, { quoted: msg})
            }
            break
            case prefix+'manga':
                if (!q) return textImg(ind.wrongFormat(prefix))
